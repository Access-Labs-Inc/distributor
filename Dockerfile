...
RUN --mount=type=cache,mode=0777,target=/home/root/app/target \
    ...
    cargo build --release --bin access-airdrop-api && cp target/release/access-airdrop-api ./
...
FROM base_image as access-airdrop-api
WORKDIR /app
COPY --from=builder /home/root/app/access-airdrop-api ./
ENTRYPOINT ["./access-airdrop-api"]
